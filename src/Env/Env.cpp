...
bool Env::addFlowerAt(const Vec2d& c) {
 ...
 bool collide=false;
 for (size_t i(0); i<hives_.size(); i++){
     if (hives_[i]->isColliding((*newflower))){
         collide=true;
         break;
     }
 }
 if (ground_->isGrowable(c) && nbflower<threshold && not collide)
 {
...
bool Env::addHiveAt(const Vec2d& position){
   ...
   if (getCollidingHive(*newhive)==nullptr and getCollidingFlower(*newhive)==nullptr){
